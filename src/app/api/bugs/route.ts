...
const CreateSchema = z.object({
  ...
  jiraId: z.string().nullable().optional().refine(val => !val || val.length <= 100, "jiraId must be less than 100 characters"),
  overview: z.string().optional().refine(val => !val || val.length <= 10000, "overview must be less than 10000 characters"),
  overviewLoginCondition: z.string().nullable().optional().refine(val => !val || val.length <= 1000, "overviewLoginCondition must be less than 1000 characters"),
  overviewPlatform: z.string().nullable().optional().refine(val => !val || val.length <= 500, "overviewPlatform must be less than 500 characters"),
  overviewModule: z.string().nullable().optional().refine(val => !val || val.length <= 500, "overviewModule must be less than 500 characters"),
  overviewTrigger: z.string().nullable().optional().refine(val => !val || val.length <= 1000, "overviewTrigger must be less than 1000 characters"),
  overviewIssue: z.string().nullable().optional().refine(val => !val || val.length <= 2000, "overviewIssue must be less than 2000 characters"),
  envPage: z.string().nullable().optional().refine(val => !val || val.length <= 500, "envPage must be less than 500 characters"),
  envPlatform: z.string().nullable().optional().refine(val => !val || val.length <= 500, "envPlatform must be less than 500 characters"),
  envOS: z.string().nullable().optional().refine(val => !val || val.length <= 200, "envOS must be less than 200 characters"),
  envBrowser: z.string().nullable().optional().refine(val => !val || val.length <= 200, "envBrowser must be less than 200 characters"),
  preconditions: z.array(z.string()).optional().refine(val => !val || (val.every(v => v.length <= 2000) && val.length <= 50), "preconditions: max 50 items, each less than 2000 characters"),
  stepsToReproduce: z.array(z.string()).optional().refine(val => !val || (val.every(v => v.length <= 2000) && val.length <= 100), "stepsToReproduce: max 100 items, each less than 2000 characters"),
  actualResult: z.string().nullable().optional().refine(val => !val || val.length <= 5000, "actualResult must be less than 5000 characters"),
  expectedResult: z.string().nullable().optional().refine(val => !val || val.length <= 5000, "expectedResult must be less than 5000 characters"),
  userImpact: z.string().nullable().optional().refine(val => !val || val.length <= 2000, "userImpact must be less than 2000 characters"),
  businessImpact: z.string().nullable().optional().refine(val => !val || val.length <= 2000, "businessImpact must be less than 2000 characters"),
  qaImpact: z.string().nullable().optional().refine(val => !val || val.length <= 2000, "qaImpact must be less than 2000 characters"),
  technicalNotes: z.string().nullable().optional().refine(val => !val || val.length <= 10000, "technicalNotes must be less than 10000 characters"),
  environmentStage: z.enum(["dev", "staging", "production"]).optional(),
  status: z.enum(["open", "closed"]).optional(),
  priority: z.enum(["low", "medium", "high", "critical"]).optional(),
  assignee: z.string().nullable().optional().refine(val => !val || val.length <= 200, "assignee must be less than 200 characters"),
  reporter: z.string().default("Anonymous").refine(val => val.length <= 200, "reporter must be less than 200 characters"),
  labelIds: z.array(z.string()).optional().refine(val => !val || val.length <= 20, "labelIds: max 20 labels"),
});
